...
## 2026-09-20: 第1章のタイムアウト改善と実生成による完成

通常suite **551 passed / 2 skipped / 5 warnings、44.88秒**。Ruff、Mypy（46 source files）、差分検査成功。スキップは任意のinstalled CLI localhost probeと実E5検証。証跡はGit管理外 `.study-runtime/chapter-timeout-recovery/regression.xml`。

### 実測した停止箇所と変更

対象の章は設計と第1〜4節が保存済みで、第5節の実装説明が1要求180秒に達して停止していた。第2〜4節は約149〜163秒、本文と補足は約6000〜6600文字。CLI内のどの計算・通信に時間がかかったかは旧ログから確定できない。

章全体の詳細設定を各節で反復しないよう指示を整理。節は担当する目標に集中し、同じ数値例を用い、数式・コードの意味を保持しながら重複説明を減らす。2500〜4000文字は目安であり、切捨て・受入条件ではない。設計時も長い章の論点を6〜8節へ分散する。1要求180秒、開始・再開全体1800秒、最大9要求、モデル/effortと実行境界は変更していない。

時間切れの診断には対象工程、節番号、上限秒数を残す。合成テストで第2節のtimeout後に成功済み第1節が保持され、次の節へ進まず、ジョブと試行に同じ診断が残ることを確認。

### ユーザーが明示依頼したコースの実生成

今回は「第1章をエラーなく生成できるまで」という追加の実行依頼に基づく本番コースの完成操作。本人の教材・答案をテストfixtureへ転用したものではない。通常の合成・mock回帰は隔離保存先で実施。本番操作前にprogress/learning/jobsをSQLite backup APIで保存した。

- 公式Codex CLI 0.155.1、ChatGPT認証、`gpt-6-astra / medium`。別モデル・effort・APIへのfallback、Reset消費なし。
- 保存済み第1〜4節を完全一致で再利用。残る第5節 **101.48秒**、第6節 **107.71秒**で成功。追加生成 **2要求**、自動retryなし。
- 第5節は数式とPython処理の対応、途中値の照合、パラメータ比較と誤りの見分け方を含む。第6節は学習/評価、MSEの数値例、汎化・外挿・データ漏洩、因果関係との区別を含む。生成コードは実行せず、教材にも未実行と明記されている。
- 本文＋補足は新規第5節4157文字、第6節3855文字。文字数を成功条件にはせず、形式・参照・節の目標と保存整合を検査。
- 6節の講義がコースに反映済み。対象コースの変更は第1章のlecture_contentだけで、修了・採点・EXPを変更しない。旧4節は同一。実ブラウザで対象の第1章、6節の見出し、講義/相談/練習/修了試験タブが表示されることを確認。
- 補助実行スクリプトの最初の起動でclaim_submissionのtuple戻り値を未展開のまま渡すエラーが発生。生成要求前に中断され、修正後に明示実行した。上記2要求に隠れた失敗生成はない。
- この依頼用の上限4要求の記録を別のGit管理外領域へ保存し、従来の実検証8ジョブの上限・履歴は変更していない。

今回確認したのは既存成功4節を再利用する第1章の完成。新規の章を設計から全節生成する成功率や、今後の全章でのタイムアウト解消を保証する結果ではない。実教材・生成全文・バックアップはGit管理外に留める。

## 2026-09-20: 講義設計の形式指定と診断の改善

通常suite **548 passed / 2 skipped / 5 warnings、45.34秒**。証跡: `.study-runtime/schema-diagnostics-tests.xml`。スキップは明示opt-inのCLI localhost probeと実E5モデル検証。5 warningsは既存PyMuPDF/SWIGの非推奨警告。

- 調査: 実ジョブのメタデータでは `lecture_plan` が2件 `schema_error`。設計保存前・本文0節。最終回答と検証パスは旧実装で保存されず、実際の違反項目は未確定。
- 合成再現: CLI送信用Schemaに適合する121文字タイトル、重複目標が、厳密な受信検査で拒否されることを確認。
- 修正: 元Schemaから省略制約を生成してProviderの入力へ補足し、追加後も入力上限を維持。存在しない補足フィールドを作らせる共通指示を除去。JSON解析とSchema違反を区別し、値を含めない許可リスト方式の診断をジョブ補助表と講義試行へ保存。
- 検証: 日本語文字数、重複、欠落、余分なキー、壊れたJSON、Markdown囲み、診断への秘密文字列非混入、ジョブ再起動後の診断復元、講義での停止・追加要求0件、UI通知と通知を閉じた後の理由表示・進捗不変を追加検証。
- 互換性: 保存形式の再移行なし。既存の要求バージョンは維持し、旧失敗には診断を捏造しない。全体suiteで既存のキャンセル、timeout、教材変更、成功節を再利用する明示再開も検証。
- 開発中の失敗: 新規UIテストの検索語が合成教材に一致せず、生成自体が始まらなかった。検索語を教材に合わせ、実際にProviderが1回呼ばれたことまで確認するテストへ修正。要求バージョンを一時的に上げるテスト変更は、旧チェックポイントの再開互換性を維持するため戻した。
- 静的検証: Ruff、Mypy（46 source files）、verify_score、HTMLの重複IDと内部アンカー検査、git diff --check。HTMLの今回追記に対する実ブラウザ目視は未実施。UIはStreamlit AppTestで確認。
- 実Codex生成、Reset、外部送信なし。本人データをテストに使用していない。実生成の成功率・個別の過去失敗の原因は未検証。既存の実検証8ジョブの上限・履歴を変更していない。
