...
## Chapter batches and editions (2026-09-20)

`LectureBatch` calls `lecture_plan` followed by up to eight `lecture_section` requests under one JobManager lock. Each request retains its 180-second Provider limit; each explicit start/resume has a 1,800-second cooperative cancellation deadline. Checkpoints and the current mirror use `LearningRepository.update_document` compare-and-update in one transaction. Successful sections, request digests and metadata are validated and reused on explicit resume; startup never replays. Chapter publication archives the previous edition and uses existing progress event receipts. No SQLite schema migration is added.

`select_edition_context` distributes bounded context across sections and objectives, or prioritizes a selected section. Practice set/feedback schemas never award scores or XP. UI visibility, draft persistence, per-question submissions, review candidates and previous-edition exports are covered by synthetic AppTest. See [LEARNING_DESIGN](LEARNING_DESIGN.md) for the full learning contract.
